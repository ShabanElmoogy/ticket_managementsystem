...
| `AppBadge` | `forms/AppBadge.tsx` | ✅ Yes (`useThemeColors`) | ❌ No — screens only | `TicketsScreen`, `TenantsScreen`, `TemplatesScreen` |
...
**⚠️ Modal rule:** `AppBadge` calls `useThemeColors()` internally. Do NOT render it inside a `<Modal>`. If needed inside a modal, pass the resolved `color` prop explicitly:
```tsx
// ✅ Safe inside Modal — color resolved by parent
const c = useThemeColors();
<AppBadge label="OPEN" color={StatusColors['OPEN']} />
```
