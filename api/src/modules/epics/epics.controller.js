...
// Attach feature count + step progress to each epic
const attachProgress = async (rows) => {
  ...
  const features = await db
    .select({ id: featureRequests.id, epicId: featureRequests.epicId })
  ...
  return rows.map((r) => {
    ...
    return {
      ...
      stepsTotal:   epicSteps.length,
      stepsDone:    epicSteps.filter((s) => s.status === 'DONE').length,
    };
  });
};
...
export const createEpic = async (req, res) => {
  try {
    ...
    res.status(201).json({ ...row, ownerName: null, applicationName: null, customerName: null, featureCount: 0, stepsTotal: 0, stepsDone: 0 });
  } catch (err) {
...
// Link / unlink a feature to an epic
